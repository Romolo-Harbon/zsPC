...
//          $DepIdS = 4;
            ...
            $data['status'] = 'fail';
//          $sql_GetMes = "select id,TabNam,TabCTm,TabDTm,CirSmp from table_mes where TabTyp = ".$TypeId." and ProAId = '".$projectId."' and TabSta = 1 order by TabDTm";
            $sql_GetMes = "select id,CirSmp,TabDTm,TabNam,TabCTm from sign_check where TabNam LIKE '%".$SearchVal."%' and TabTyp = ".$TypeId." and ProAId = '".$projectId."' and TabSta = 1 and DepIdS = '".$DepIdS."' and cast(TabDTm  as datetime) <= '$showtime' order by TabDTm";
            $result_GetMes = $conn->query($sql_GetMes);
            if($result_GetMes->num_rows>0)
            {
                $i = 0;
